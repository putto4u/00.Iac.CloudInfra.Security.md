...
# 5. GPG 키 및 키링 관리 구절 (보안 및 확장성)
# [주석] 나중에 외부 저장소(Docker, Google Cloud 등)를 추가할 때 
# 이 위치에 해당 서비스의 GPG 키를 추가하는 로직을 넣으시면 됩니다.
RUN install -m 0755 -d /etc/apt/keyrings && \
    # 예시: 도커 공식 GPG 키 등록 (추후 확장을 위한 기초 설정)
    curl -fsSL https://download.docker.com/linux/ubuntu/gpg | gpg --dearmor -o /etc/apt/keyrings/docker.gpg && \
    chmod a+r /etc/apt/keyrings/docker.gpg

# 6. pyenv 빌드 의존성 및 컴파일 도구 설치
RUN apt-get update && apt-get install -y \
...
# 7. pyenv 설치 및 특정 파이썬 버전(3.12.1) 빌드
RUN curl https://pyenv.run | bash && \
    pyenv install 3.12.1 && \
    pyenv global 3.12.1

# 8. 작업 디렉토리 및 쉘 환경 자동화 설정
WORKDIR /app
